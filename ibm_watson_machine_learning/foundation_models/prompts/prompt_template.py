...
class PromptTemplate:
    ...
    **Examples**
    ...
    def __init__(self,
                 ...
                 examples: Optional[List[List[str]]] = None) -> None:
        self.name = name
...
class PromptTemplateManager(WMLResource):
    ...
    **Example**
    ...
    def _from_json_to_prompt(self, response: Dict) -> PromptTemplate:
        ...
        # To bypass validation step
        prompt_template.input_variables = response.get('prompt_variables')
        prompt_template.input_text = prompt_field.get('input', [[None, None]])[0][0]
        prompt_template.instruction = data_field.get('instruction')
        prompt_template.input_prefix = data_field.get('input_prefix')
        prompt_template.output_prefix = data_field.get('output_prefix')
        prompt_template.examples = data_field.get('examples')
