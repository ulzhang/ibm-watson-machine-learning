...
class Deployments(WMLResource):
    ...
    def _get_model_inference(self, deployment_id, item, params=None):
        ...
        generated_url = self._client.service_instance._href_definitions.get_fm_deployment_generation_href(
            deployment_id=deployment_id, item=item)

        if generated_url not in self.get_details(deployment_id, _silent=True)['entity'].get('status', {}).get('serving_urls', []):
            raise WMLClientError(f"Deployment with id:{deployment_id} does not have inferences with \'{item}\'.")
